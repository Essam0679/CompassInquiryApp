// --- START OF FILE app/edit-profile.tsx (Full Code - Adapted to your UI) ---
...
import { useAuth, UserProfile } from '@/context/AuthContext'; // Use UserProfile
import { useRouter } from 'expo-router';
import Header from '@/components/common/Header';
import DropDownPicker, { ItemType } from 'react-native-dropdown-picker';
import { gccCountryOptionsForRegistration } from '@/config/contacts';
import { updateProfile as firebaseUpdateProfile } from 'firebase/auth'; // For updating Firebase Auth display name
...
export default function EditProfileScreen() {
  ...
  useEffect(() => {
    if (userProfile || currentUser) {
      setFormData({
        ...
        // Email is typically not edited here, but displayed from currentUser.email
  ...
  const handleSubmit = async () => {
    setCountryOpen(false); setBusinessTypeOpen(false);
    if (!validateForm()) {
    ...
    try {
      ...
      const currentDisplayName = userProfile?.name || currentUser?.displayName;

      if (formData.name !== currentDisplayName) updatedDataForFirestore.name = formData.name;
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" scrollEnabled={!businessTypeOpen && !countryOpen}>
        ...
        <View style={styles.formSection}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput style={[styles.textInput, { backgroundColor: colors.card, color: colors.text, borderColor: errors.name ? colors.error : colors.border }]} value={formData.name} onChangeText={(text) => handleInputChange('name', text)} placeholderTextColor={colors.textSecondary} />
          ...
          <View style={styles.inputGroup}>
            <Text style={[styles.inputLabel, { color: colors.text }]}>{t('phone', {defaultValue: 'Phone Number'})}</Text>
            <TextInput style={[styles.textInput, { backgroundColor: colors.card, color: colors.text, borderColor: errors.phone ? colors.error : colors.border }]} value={formData.phone} onChangeText={(text) => handleInputChange('phone', text)} keyboardType="phone-pad" placeholderTextColor={colors.textSecondary} />
        ...
        <View style={styles.formSection}>
          ...
          <View style={styles.inputGroup}>
            <Text style={[styles.inputLabel, { color: colors.text }]}>{t('companyName', {defaultValue: 'Company Name'})}</Text>
            <TextInput style={[styles.textInput, { backgroundColor: colors.card, color: colors.text, borderColor: colors.border }]} value={formData.companyName} onChangeText={(text) => handleInputChange('companyName', text)} placeholderTextColor={colors.textSecondary} />
          </View>
        ...
        <TouchableOpacity style={[styles.saveButton, { backgroundColor: colors.primary, opacity: loading || contextAuthLoading ? 0.7 : 1 }]} onPress={handleSubmit} disabled={loading || contextAuthLoading}>
          {(loading || contextAuthLoading) ? <ActivityIndicator color={colors.white || '#FFFFFF'} size="small" /> : <Text style={[styles.saveButtonText, { color: colors.white || '#FFFFFF' }]}>{t('save', {defaultValue: 'Save Changes'})}</Text>}
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  textInput: { flex: 1, height: 56, borderRadius: 8, paddingLeft: 16, paddingRight: 16, fontFamily: 'Inter-Regular', fontSize: 15, borderWidth: 1, },
  inputIcon: { marginRight: 10 }, // Adjusted if icons are outside TextInput
  disabledInput: { opacity: 0.7 }, 
  errorText: { fontFamily: 'Inter-Regular', fontSize: 12, marginTop: 6, marginLeft: 2, color: 'red' },
...
// --- END OF FILE app/edit-profile.tsx (Full Code - Adapted to your UI) ---