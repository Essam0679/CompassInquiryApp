...
=======
  ...
  const containerTypes = [
    { label: t('fcl'), value: 'fcl' },
    { label: t('lcl'), value: 'lcl' }
  ];
  
  const containerSizes = [
    { label: '20\' Standard', value: '20ft' },
    { label: '20\' High Cube', value: '20hc' },
    { label: '20\' Open Top', value: '20ot' },
    { label: '20\' Flat Rack', value: '20fr' },
    { label: '20\' Platform', value: '20pl' },
    { label: '20\' Refrigerated', value: '20rf' },
    { label: '40\' Standard', value: '40ft' },
    { label: '40\' High Cube', value: '40hc' },
    { label: '40\' Open Top', value: '40ot' },
    { label: '40\' Flat Rack', value: '40fr' },
    { label: '40\' Platform', value: '40pl' },
    { label: '40\' Refrigerated', value: '40rf' },
    { label: '45\' High Cube', value: '45hc' },
    { label: t('leaveToCompany'), value: 'any' }
  ];
      ...
      form: (
        <View style={styles.formStep}>
          ...
          <DropDownPicker
            ...
            setOpen={setContainerTypeOpen}
            setValue={(callback) => {
              if (typeof callback === 'function') {
                const value = callback(formData.containerType);
                handleInputChange('containerType', value);
              } else {
                handleInputChange('containerType', callback);
              }
            }}
            style={[styles.dropdown, { backgroundColor: colors.backgroundSecondary, borderColor: colors.border }]}
            dropDownContainerStyle={[styles.dropdownContainer, { backgroundColor: colors.backgroundSecondary, borderColor: colors.border }]}
            textStyle={[styles.dropdownText, { color: colors.text }]}
            placeholder={t('selectContainerType')}
            zIndex={2000}
            zIndexInverse={2000}
          />
          
          {formData.containerType === 'fcl' && (
            <View style={{ marginTop: 16, marginBottom: containerSizeOpen ? 200 : 0 }}>
              <DropDownPicker
                open={containerSizeOpen}
                value={formData.containerSize}
                items={containerSizes}
                setOpen={setContainerSizeOpen}
                setValue={(callback) => {
                  if (typeof callback === 'function') {
                    const value = callback(formData.containerSize);
                    handleInputChange('containerSize', value);
                  } else {
                    handleInputChange('containerSize', callback);
                  }
                }}
                style={[styles.dropdown, { backgroundColor: colors.backgroundSecondary, borderColor: colors.border }]}
                dropDownContainerStyle={[styles.dropdownContainer, { backgroundColor: colors.backgroundSecondary, borderColor: colors.border }]}
                textStyle={[styles.dropdownText, { color: colors.text }]}
                placeholder={t('selectContainerSize')}
                zIndex={1000}
                zIndexInverse={3000}
                maxHeight={300}
              />
...
=======
    {
      ...
      form: (
        <View style={styles.formStep}>
          ...
          <View style={styles.inputGroup}>
            <Text style={[styles.inputLabel, { color: colors.text }]}>
              {formData.shipmentType === 'export' ? t('originPort') : t('destinationPort')}
            </Text>
            <TextInput
              style={[styles.textInput, { backgroundColor: colors.backgroundSecondary, color: colors.text, borderColor: colors.border }]}
              placeholder={formData.shipmentType === 'export' ? t('enterOriginPort') : t('enterDestinationPort')}
              placeholderTextColor={colors.textSecondary}
              value={formData.shipmentType === 'export' ? formData.originPort : formData.destinationPort}
              onChangeText={(text) => handleInputChange(formData.shipmentType === 'export' ? 'originPort' : 'destinationPort', text)}
            />
          </View>
          ...
          <View style={styles.inputGroup}>
            <Text style={[styles.inputLabel, { color: colors.text }]}>
              {formData.shipmentType === 'export' ? t('destinationPort') : t('originPort')}
            </Text>
            <TextInput
              style={[styles.textInput, { backgroundColor: colors.backgroundSecondary, color: colors.text, borderColor: colors.border }]}
              placeholder={formData.shipmentType === 'export' ? t('enterDestinationPort') : t('enterOriginPort')}
              placeholderTextColor={colors.textSecondary}
              value={formData.shipmentType === 'export' ? formData.destinationPort : formData.originPort}
              onChangeText={(text) => handleInputChange(formData.shipmentType === 'export' ? 'destinationPort' : 'originPort', text)}
            />
          </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <Header title={t('seaFreightQuote')} showBackButton={true} />
